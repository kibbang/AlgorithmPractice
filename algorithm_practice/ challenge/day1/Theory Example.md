...
빅오 표기법이 알고리즘 효율을 판단하는 기준이 되는 이유는 알고리즘의 효율의 판단은 적은 양의 데이터가 아닌 많게는 수백 수천만 건의 데이터를 가지고 판단을 하기 때문이다. 그렇기 때문에 초반에는 효율이 떨어질 수 있어도 일정 구간을 넘어가면서부터는 효율의 격차를 뒤집을 수 없게 된다.
2.수학적 그래프로 예를들면 값이 커질수록 그래프의 기울기 상승이 크게 증가하는데 이때 최고차 항이 아닌 항과 상수항은 이 증가에 크게 영향을 미치지 않기 때문이다.

 

 3. O(1) -> O(logn) -> O(n) -> O(nlogn) -> O(n^2) -> O(2^n)

O(1) -> set에서 key 찾기(1번의 연산으로 가능)

O(logn) ->이진탐색 (연산을 진행할 수록 데이터양이 절반으로 감소)

O(n) ->리스트or 스택에서 인덱스 찾기 (선형구조에서 최악의 경우는 n번 탐색)

O(nlogn) -> merge sort

O(n^2) -> 데이터가 늘어날때 연산 실행 시간이 데이터의 제곱이 됨 이중 루프 구조 or buble sort

O(2^n) ->데이터가 하나 늘어날때마다 시간이 2배씩 증가 (파보나치)
...
빅오(Big-O) 표기법은 점근적 상한을 나타냅니다. 즉, 입력 크기 n이 아주 커질 때 연산 횟수가 어떻게 증가하는지를 나타내는 지표입니다.
...
5/n² → 0에 수렴 (사라짐)
결국 남는 것은 상수 3뿐입니다.
이것이 의미하는 바는, n이 매우 커질 때 전체 식의 값은 오직 n² 항의 변화 추이와 가장 유사하게 움직인다는 것입니다. 10n이나 5 같은 항들은 n²이 커지는 속도에 비하면 '먼지'와 같이 미미한 존재가 되어버립니다.
